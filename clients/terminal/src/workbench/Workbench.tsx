...
function TabHost(props: IDockviewPanelProps) {
  ...
  if (!Comp) return <div style={{ padding: 24, color: "var(--t3)", fontSize: 13 }}>Tipo de aba desconhecida: {kind}</div>;
...
function TabHeader(props: IDockviewPanelHeaderProps) {
  ...
  const pinItem = pinned
    ? { id: "unpin-tab", label: "Desafixar aba", onSelect: () => layout.unpinTab(props.api.id) }
    : { id: "pin-tab", label: "Fixar aba", detail: "manter aberta", onSelect: () => layout.pinTab(props.api.id) };
  const copyItems = params.kind === "doc" && path
    ? [
      { id: "copy-reference", label: "Copiar referência", detail: `@file:${path}`, onSelect: () => copyText(`@file:${path}`) },
      { id: "copy-path", label: "Copiar caminho", detail: path, onSelect: () => copyText(path) },
    ]
    : params.kind === "meeting" && meetingId
      ? [{ id: "copy-reference", label: "Copiar referência", detail: `@meeting:${meetingId}`, onSelect: () => copyText(`@meeting:${meetingId}`) }]
      : [];
  return (
    <div
    ...
    >
      ...
      <span role="button" aria-label={pinned ? "Desafixar aba" : "Fixar aba"}
        title={pinned ? "Fixada — clique para desafixar" : "Fixar aba (mantém aberta)"}
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "var(--sidebar)", borderRight: "1px solid var(--line)", minHeight: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "12px 14px 8px", flex: "none" }}>
        ...
        <img src="/vexa-logo.svg" alt="Master Meeting" width={24} height={24} style={{ borderRadius: 7, display: "block", flex: "none" }} />
        <span style={{ fontSize: 13, fontWeight: 500, color: "var(--t1)" }}>Master <span style={{ fontWeight: 400, color: "var(--t3)" }}>Meeting</span></span>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 2, padding: "2px 8px 8px", borderBottom: "1px solid var(--line)", flex: "none" }}>
        {lists.map((l) => (
          <button key={l.id} style={seg(l.id === active?.id)}
            ...
            {l.id === "files" && badge > 0 && (
              <span title={`${badge} nova(s) atualização(ões) de outros membros`}
  ...
  return (
    <div style={{ padding: "8px 12px", borderTop: "1px solid var(--line)", display: "flex", alignItems: "center", gap: 9, flex: "none" }}>
      ...
      <button type="button" title="Configurações"
        onClick={() => layout.openTab({ id: "settings", title: "Configurações", kind: "settings", params: {} })}
...
export function Workbench() {
  ...
  const resolveFirstView = async (fresh: boolean) => {
    ...
    const openMeeting = (mid: string, reveal: boolean) => {
      ...
      layout.openTab({ id: `meeting:${mid}`, title: "Reunião compartilhada", kind: "meeting", params: { meetingId: mid } });
    };
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)", color: "var(--t1)" }}>
      ...
      <footer style={{ height: 24, flex: "none", background: "var(--sidebar)", borderTop: "1px solid var(--line)", display: "flex", alignItems: "center", fontSize: 11.5, color: "var(--t2)" }}>
        <div style={{ flex: 1 }} />
        <button onClick={() => layout.resetLayout()} style={{ padding: "0 10px", height: "100%", background: "none", border: "none", color: "var(--t3)", cursor: "pointer" }} title="Redefinir layout">redefinir layout</button>
      </footer>
