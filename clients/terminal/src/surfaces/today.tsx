...
export function deviationPhrase(g: MeetingGroup, hasOwnBrief = false): { text: string; tone: "live" | "danger" | "accent" } | null {
  const m = g.current;
  if (g.phase === "live") return { text: "em reunião →", tone: "live" };
  if (m.auto_join_error) return { text: "não foi possível importar — sem link da reunião", tone: "danger" };
  if (!m.native_id && !m.meeting_url) return { text: "sem link da reunião", tone: "danger" };
  if (!m.workspace_id && !hasOwnBrief) return { text: "sem resumo ainda", tone: "accent" };
  return null;
}
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "64px 1fr", gap: "0 14px", padding: "11px 14px",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 0 }}>
        {day.groups.length === 0
          ? <span style={{ fontSize: 12.5, color: "var(--t3)", padding: "2px 0" }}>Nenhum evento hoje</span>
...
function PastLine({ m }: { m: MeetingMock }) {
  ...
  useEffect(() => {
    ...
    fetchDurableTranscript(m.id)
      .then((t) => {
        if (!on) return;
        if (t.notes.length) setLine(`${t.notes.length} anotações`);
        else if (t.lines?.length) setLine(`${t.lines.length} linhas`);
      })
      .catch(() => {});
    return () => { on = false; };
...
function PastRow({ e, reviewedIds }: { e: PastEntry; reviewedIds: Set<string> }) {
  ...
  const phrase = run.has_recording
    ? (unreviewed ? { text: "recapitulativo pronto", color: "var(--accent)" } : null)
    : { text: "nada capturado", color: "var(--t3)" };
  return (
    <div onClick={open}
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "18px 22px" }}>
      <div style={{ maxWidth: 720 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 17, fontWeight: 700, color: "var(--t1)", flex: 1 }}>Próximas reuniões</span>
          {weekOffset > 0 && (
        ...
        {empty ? (
          ...
          <>
            ...
            <div style={{ marginTop: 14, border: "1px solid var(--line)", borderRadius: 12, background: "var(--panel)" }}>
              {days.length === 0
                ? <div style={{ padding: "14px 16px", fontSize: 12.5, color: "var(--t3)" }}>Nada esta semana.</div>
        ...
        {!empty && (
          <div style={{ fontSize: 11.5, color: "var(--t3)", margin: "24px 2px 10px", lineHeight: 1.5 }}>
            Reuniões mais antigas estão no Conhecimento — pergunte ao agente sobre qualquer coisa dita ou decidida.
          </div>
        )}
      </div>
    </div>
