...
export type ChatStreamRequest = {
  ...
  context?: unknown;
  /** the explicitly requested model, or undefined */
  model?: string;
  /** the explicitly requested provider, or undefined */
  provider?: string;
};
  ...
  const drainOnce = async (): Promise<"closed" | "terminal"> => {
    ...
    try {
      r = await fetchImpl("/api/chat", {
        ...
        body: JSON.stringify({ prompt: req.prompt, session: req.session, active: req.active, context: req.context, turn_id: turnId, model: req.model, provider: req.provider }),
        signal,
      });
    } catch (e) {
