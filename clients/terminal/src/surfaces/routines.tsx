...
const BOARD: TabDescriptor = { id: "board:routines", title: "Rotinas", kind: "routines", params: {}, context: null };
...
function RoutinesBoardNav() {
  ...
  return (
    <button onClick={nav.onClick} onDoubleClick={nav.onDoubleClick} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 9px", borderRadius: 7, border: "1px solid var(--line2)", background: "var(--panel)", color: "var(--t1)", fontSize: 13, cursor: "pointer", marginBottom: 8 }}>
      <Icon name="zap" size={14} />Quadro de rotinas
    </button>
  );
}
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", background: "var(--bg)" }}>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "24px" }}>
        <div style={{ fontSize: 18, color: "var(--t1)", fontWeight: 500, marginBottom: 4 }}>Rotinas</div>
        <div style={{ fontSize: 13, color: "var(--t3)", marginBottom: 20 }}>Agentes agendados. Crie um no Chat com <code style={{ fontFamily: "var(--mono)", color: "var(--accent)" }}>/routine</code>; gerencie-os aqui.</div>
        {error && <div role="alert" style={{ fontSize: 12.5, color: "var(--danger)", background: "var(--panel)", border: "1px solid var(--danger)", borderRadius: 8, padding: "8px 11px", marginBottom: 14 }}>⚠ Não foi possível carregar as rotinas — {error}</div>}
        {routines.map((r) => (
          <div key={r.id} style={{ border: "1px solid var(--line)", borderRadius: 12, background: "var(--panel)", padding: "14px 16px", marginBottom: 12, opacity: r.enabled ? 1 : 0.55 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={sw(!!r.enabled)} onClick={() => void toggle(r)} title={r.enabled ? "Ativado" : "Desativado"}><div style={knob(!!r.enabled)} /></div>
              <button onClick={() => setEditing(editing === r.id ? null : r.id)} title="Editar" style={{ background: "none", border: "none", color: "var(--t3)", cursor: "pointer", display: "flex" }}><Icon name="panel" size={14} /></button>
              <button onClick={() => void del(r.id)} title="Excluir" style={{ background: "none", border: "none", color: "var(--t3)", cursor: "pointer", display: "flex" }}><Icon name="x" size={14} /></button>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 9 }}>
              <span style={{ fontSize: 11, color: "var(--t3)" }}>agendamento</span>
              {editing === r.id
        ...
        {routines.length === 0 && <div style={{ color: "var(--t3)", fontSize: 13, padding: "20px 0" }}>Nenhuma rotina ainda — abra o Chat e tente <code style={{ fontFamily: "var(--mono)", color: "var(--accent)" }}>/routine</code>.</div>}
      </div>
    </div>
  );
}
...
function RoutinesLeft() {
  ...
  return (
    <div style={{ padding: "8px" }}>
      <RoutinesBoardNav />
      <div style={{ fontSize: 11, color: "var(--t3)", textTransform: "uppercase", letterSpacing: ".04em", padding: "6px 4px 4px" }}>agentes agendados</div>
      {routines.map((r) => <RoutineNavRow key={r.id} routine={r} />)}
      {routines.length === 0 && <div style={{ padding: "8px 4px", color: "var(--t3)", fontSize: 12 }}>Nenhuma ainda — crie com <code style={{ fontFamily: "var(--mono)", color: "var(--accent)" }}>/routine</code> no Chat.</div>}
    </div>
  );
}
...
if (!meetingsOnly()) {
  ...
  registerList({ id: "routines", label: "Rotinas", icon: "zap", order: 40, component: RoutinesLeft });
}
