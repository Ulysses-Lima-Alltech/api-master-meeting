...
function ChatHeader({ subject, session, onSelectSession, onNewChat, onClose, aiProvider, setAiProvider }: {
  subject: string;
  session: string;
  onSelectSession: (id: string) => void;
  onNewChat: () => void;
  onClose: () => void;
  aiProvider: { name: string; model: string; provider: string; icon?: string };
  setAiProvider: (p: { name: string; model: string; provider: string; icon?: string }) => void;
}) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") { 
        ...
        setOpen(false); 
        setModelOpen(false);
      }  // consume: close-topmost beats nav.back
    };
  ...
  const models = [
    { name: "ChatGPT 4o", model: "gpt-4o", provider: "openai-compat", icon: "sparkles" },
    { name: "Claude 3.5", model: "claude-3-5-sonnet-20241022", provider: "claude-code", icon: "terminal" },
    { name: "Gemini 1.5", model: "gemini-1.5-pro-latest", provider: "openai-compat", icon: "stars" },
  ];
  ...
  return (
    <div ref={menuRef} style={{ height: 38, flex: "none", position: "relative", display: "flex", alignItems: "center", gap: 4, padding: "0 8px", borderBottom: "1px solid var(--line)", background: "var(--panel)", minWidth: 0 }}>
      <button
        ...
        aria-expanded={open}
        onClick={() => { setOpen((v) => !v); setModelOpen(false); }}
      ...
      <button
        ...
        onClick={() => { setModelOpen((v) => !v); setOpen(false); }}
        style={{ height: 28, borderRadius: 7, border: "1px solid var(--line2)", background: modelOpen ? "var(--panel2)" : "var(--bg)", color: "var(--t1)", display: "flex", alignItems: "center", gap: 6, padding: "0 8px", cursor: "pointer", fontSize: 12, fontWeight: 500, flex: "none" }}
      >
        <Icon name={aiProvider.icon || "sparkles"} size={13} style={{ color: "var(--accent)" }} />
        <span>{aiProvider.name}</span>
        <Icon name="chevR" size={10} style={{ color: "var(--t3)", transform: modelOpen ? "rotate(-90deg)" : "rotate(90deg)", transition: "transform .12s" }} />
      </button>
      ...
      {modelOpen && (
        <div role="menu" style={{ position: "absolute", zIndex: 30, top: 36, right: 8, width: 220, maxHeight: 260, overflowY: "auto", border: "1px solid var(--line)", borderRadius: 8, background: "var(--panel)", boxShadow: "0 14px 34px rgba(0,0,0,.32)", padding: 4 }}>
          {models.map((m) => {
            const active = m.model === aiProvider.model;
            return (
              <button
                key={m.model}
                role="menuitemradio"
                aria-checked={active}
                onClick={() => { setAiProvider(m); setModelOpen(false); }}
                style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", border: "none", borderRadius: 6, background: active ? "var(--panel2)" : "transparent", color: active ? "var(--t1)" : "var(--t2)", cursor: "pointer", textAlign: "left", fontSize: 13 }}
              >
                <Icon name={m.icon} size={14} style={{ color: active ? "var(--accent)" : "var(--t3)" }} />
                <span style={{ minWidth: 0, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: active ? 600 : 400 }}>{m.name}</span>
...
export function Chat({ params = {} }: ChatProps) {
  const [aiProvider, setAiProvider] = useState<{ name: string; model: string; provider: string; icon?: string }>({
    name: "ChatGPT 4o",
    model: "gpt-4o",
  ...
  const send = async (text: string, prompt = text, referenceSource = text, opts: { hidden?: boolean; ground?: boolean } = {}) => {
    ...
    try {
      const result = await streamChatTurn(
        { prompt: p, session: sessionForSend, active, context, model: aiProvider.model, provider: aiProvider.provider },
        {
  ...
  return (
    <AgentWindow top={<ChatHeader subject={subject} session={session} onSelectSession={selectSession} onNewChat={newChat} onClose={() => layout.toggleRight()} aiProvider={aiProvider} setAiProvider={setAiProvider} />} scrollRef={scrollRef} composer={composer}>
