...
const roleLabel = (role: string) => (role === "viewer" ? "somente leitura" : "leitura e gravação");
 ...
 *  workspace README (pinned); "Not now" drops the invite and opens Master Meeting. */
function InviteConsent({ token, onProceed, onDecline, busy = false }: { token: string; onProceed: () => void; onDecline: () => void; busy?: boolean }) {
  ...
  useEffect(() => {
    ...
    previewInvite(token)
      ...
      .catch(() => { if (live) setErr("Este link de convite é inválido ou expirou."); });
    return () => { live = false; };
  }, [token]);
  ...
  return (
    <div style={wrap}>
      <div style={card}>
        <div style={{ fontSize: 11.5, textTransform: "uppercase", letterSpacing: ".08em", color: "var(--t3)", marginBottom: 12 }}>Você foi convidado para um workspace</div>
        {!pv && !err && <div style={{ color: "var(--t3)", fontSize: 13 }}>Carregando convite…</div>}
        {invalid && (
          <>
            <div style={{ fontSize: 16, color: "var(--t1)", marginBottom: 6 }}>Convite indisponível</div>
            <div style={{ fontSize: 13, color: "var(--t3)", marginBottom: 20 }}>{err || "Este convite não é mais válido — pode ter expirado, sido revogado ou já utilizado."}</div>
            <button onClick={onDecline} style={btnGhost}>Continuar para o Master Meeting</button>
          </>
        )}
        {pv && pv.valid && (
          <>
            ...
            <div style={{ fontSize: 13.5, color: "var(--t2)", lineHeight: 1.5, marginBottom: 18 }}>
              {pv.purpose || <span style={{ color: "var(--t3)" }}>Um workspace de conhecimento compartilhado.</span>}
            </div>
            <div style={termRow}><span style={termK}>Seu acesso</span><span style={termV}>{roleLabel(pv.role)}</span></div>
            {pv.shared_by && <div style={termRow}><span style={termK}>Compartilhado por</span><span style={termV}>{pv.shared_by}</span></div>}
            <div style={{ fontSize: 12, color: "var(--t3)", lineHeight: 1.5, margin: "16px 0 20px", paddingTop: 14, borderTop: "1px solid var(--line)" }}>
              Ao entrar, este workspace será adicionado e montado no agente. Você pode desativá-lo ou sair a qualquer momento.
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={onProceed} disabled={busy} style={{ ...btnPrimary, opacity: busy ? 0.6 : 1, cursor: busy ? "default" : "pointer" }}>{busy ? "Entrando…" : "Continuar para entrar"}</button>
              <button onClick={onDecline} disabled={busy} style={btnGhost}>Agora não</button>
            </div>
          </>
        )}
      </div>
