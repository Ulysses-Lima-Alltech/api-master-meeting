...
function localRequestOnly(request: NextRequest): NextResponse | null {
  if (process.env.MASTER_API_PANEL_ALLOW_REMOTE === "true") {
    return null;
  }
  ...
  return NextResponse.json(
    {
      error:
        "O painel administrativo aceita apenas acesso local.",
    },
    {
      status: 403,
      headers: NO_STORE,
...
async function adminRequest<T>(path: string, init: RequestInit = {}): Promise<AdminResult<T>> {
  ...
  if (!cfg) {
    return {
      ok: false,
      status: 503,
      error: "Admin API nÃ£o configurada no servidor (VEXA_ADMIN_API_URL / VEXA_ADMIN_API_KEY).",
    };
  }
...
async function ensureClient(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ error: "Corpo JSON invÃ¡lido." }, { status: 400, headers: NO_STORE });
  }

  if (!validEmail(body.email)) {
    return NextResponse.json({ error: "Informe um e-mail vÃ¡lido." }, { status: 400, headers: NO_STORE });
  }
  ...
  if (found.status !== 404) return jsonError(found, "NÃ£o foi possÃ­vel localizar o cliente.");
  ...
  if (!created.ok || !created.data) return jsonError(created, "NÃ£o foi possÃ­vel criar o cliente.");
...
async function listTokens(userId: string): Promise<NextResponse> {
  ...
  if (!listed.ok) return jsonError(listed, "NÃ£o foi possÃ­vel listar as chaves.");
...
async function mintToken(request: NextRequest, userId: string): Promise<NextResponse> {
  ...
  if (scopes.length === 0 || scopes.some((scope) => !VALID_SCOPES.has(scope))) {
    return NextResponse.json(
      { error: "Selecione ao menos um escopo vÃ¡lido: bot, tx ou browser." },
  ...
  if (!minted.ok || !minted.data?.token) return jsonError(minted, "NÃ£o foi possÃ­vel gerar a chave.");
...
async function revokeToken(request: NextRequest, tokenId: string): Promise<NextResponse> {
  ...
  if (!userId) {
    return NextResponse.json(
      { error: "userId Ã© obrigatÃ³rio para validar a propriedade da chave." },
      { status: 400, headers: NO_STORE },
    );
  }
  ...
  if (!listed.ok) return jsonError(listed, "NÃ£o foi possÃ­vel validar a chave.");

  const belongsToClient = (listed.data ?? []).some((token) => String(token.id) === tokenId);

  if (!belongsToClient) {
    return NextResponse.json(
      { error: "Chave nÃ£o encontrada para este cliente." },
  ...
  if (!revoked.ok) return jsonError(revoked, "NÃ£o foi possÃ­vel revogar a chave.");
...
async function dispatch(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  return NextResponse.json(
    { error: "Rota administrativa nÃ£o encontrada." },
    { status: 404, headers: NO_STORE },
  );
}
