...
function proxyToGateway(client, clientSocket, apiKey) {
  // If we're chaining to a remote terminal proxy, the upstream WebSocket is at /ws, not /api/ws.
  const targetPath = GATEWAY_URL.endsWith("/api") ? GATEWAY_URL.replace(/\/api$/, "/ws") : `${GATEWAY_URL}/ws`;
  const target = targetPath.replace(/^http/, "ws");
