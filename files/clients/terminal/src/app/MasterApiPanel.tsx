...
const SCOPE_OPTIONS: Array<{ id: Scope; label: string; description: string }> = [
  { id: "bot", label: "Entrar em reuniões", description: "Iniciar, acompanhar e encerrar bots." },
  { id: "tx", label: "Ler transcrições", description: "Consultar reuniões e transcrições." },
  { id: "browser", label: "Navegação web", description: "Acesso ao navegador remoto quando necessário." },
];
  ...
  return (
    <main className="shell">
      ...
      <section className="hero">
        ...
        <h1>Crie e gerencie acessos à API</h1>
        <p className="lead">
          Localize ou crie um cliente pelo e-mail, gere chaves com permissões específicas e revogue
          acessos quando necessário. O segredo administrativo permanece somente no servidor.
        </p>
      </section>
      ...
      <div className="grid">
        <section className="card">
          <div className="cardHeader">
            ...
            <div>
              ...
              <p>Digite o e-mail. Caso não exista, o cadastro será criado automaticamente.</p>
            </div>
          </div>
          ...
          <div className="row">
            ...
            <button className="button primary" disabled={!email.trim() || busyClient} onClick={() => void selectClient()}>
              {busyClient ? "Carregando…" : "Localizar ou criar"}
            </button>
          </div>
          ...
          {client && (
            <div className="clientBox">
              ...
              <div>
                <span className="muted">Reuniões simultâneas</span>
                <strong>{client.max_concurrent_bots ?? "padrão"}</strong>
              </div>
        ...
        <section className={`card ${client ? "" : "disabledCard"}`}>
          <div className="cardHeader">
            ...
            <div>
              <h2>Nova chave</h2>
              <p>Defina um nome, as permissões e a validade do acesso.</p>
            </div>
          </div>
          ...
          <div className="label scopeTitle">Permissões</div>
          ...
          >
            {busyToken ? "Gerando chave…" : "Gerar chave de API"}
          </button>
        </section>
      </div>
      ...
      {mintedToken && (
        <section className="secretCard">
          <div>
            ...
            <h2>Copie agora — ela não será exibida novamente</h2>
          </div>
      ...
      <section className={`card tokensCard ${client ? "" : "disabledCard"}`}>
        <div className="cardHeader">
          ...
          <div>
            ...
            <p>Os valores secretos não são recuperáveis; somente os metadados ficam disponíveis.</p>
          </div>
          {client && (
        ...
        {!client ? (
          ...
          <div className="empty">Este cliente ainda não possui chaves.</div>
        ) : (
          <div className="tokenTable">
            {tokens.map((token) => (
              <article className="tokenRow" key={token.id}>
                ...
                >
                  {busyRevoke === token.id ? "Revogando…" : "Revogar"}
                </button>
              </article>
            ))}
          </div>
      ...
      <footer>
        <span>
          Painel sem login de usuário e restrito a <strong>localhost</strong>. Não exponha a porta
          13000 publicamente sem autenticação administrativa.
        </span>
        <a href="/terminal">Abrir Terminal legado</a>
      </footer>
