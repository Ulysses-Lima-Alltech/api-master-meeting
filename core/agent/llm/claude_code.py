...
class ClaudeCodeHarness:
    ...
    def run_turn(self, work: Path, prompt: str, *, allowed_tools: Iterable[str] = (),
        ...
        # [VEXA INTELLIGENCE] Gracefully handle users providing OpenAI keys to the Anthropic runner
        anthropic_key = os.environ.get("ANTHROPIC_API_KEY", "")
        if anthropic_key.startswith("sk-proj-"):
            yield {
                "type": "message-delta", 
                "text": "⚠️ **Aviso de Configuração:** O Agente Lateral do Vexa utiliza a tecnologia oficial `claude-code`, que **só aceita chaves da Anthropic (Claude)**. Identificamos que você configurou uma chave da OpenAI (`sk-proj-`).\n\nPara usar este chat, gere uma chave em console.anthropic.com e atualize a variável `ANTHROPIC_API_KEY` no painel do Railway."
            }
            yield {
                "type": "done",
                "ok": False,
                "reply": "Anthropic API key required. OpenAI key detected.",
                "sessionId": session,
                "detail": "Please configure a valid ANTHROPIC_API_KEY to use the agent chat."
            }
            return
