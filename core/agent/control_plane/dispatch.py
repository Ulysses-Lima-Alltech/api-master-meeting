...
def build_unit_env(settings: Settings, invocation: dict, *, unit_id: str, token: str,
    ...
    # [VEXA INTELLIGENCE] Override model if explicitly requested by the UI side agent
    ctx = invocation.get("context") or {}
    if ctx.get("model_override"):
        env["VEXA_AGENT_MODEL"] = ctx["model_override"]
...
def _without_chat_session(invocation: dict) -> dict:
    ...
    needs_clean = has_principal or (ctx_dict is not None and (
        "session" in ctx_dict or "model_override" in ctx_dict or (isinstance(meeting, dict) and bool(_INTERNAL_MEETING_HINTS & meeting.keys()))
    ))
    if not needs_clean:
        return invocation
    ...
    if ctx_dict is not None:
        clean_ctx = {k: v for k, v in ctx_dict.items() if k not in ("session", "model_override")}
