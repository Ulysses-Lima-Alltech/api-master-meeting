...
def _handle(r, dispatcher, live, subject, p, last_arm, keymap, first_seen) -> None:
    ...
    opt_in = r.get(f"proc:meeting:{key}:on")
    
    # [VEXA INTELLIGENCE] Default to ON automatically if not explicitly turned off!
    if opt_in is None:
        opt_in = "1"
        try:
            r.set(f"proc:meeting:{key}:on", "1")
        except Exception:
            pass

    if opt_in and str(opt_in) != "0" and now - last_arm.get(key, 0.0) > REARM_SEC:
        last_arm[key] = now
