...
# --------------------------------------------------------------------------- #
# sales intelligence tables (SalesMeet features)
# --------------------------------------------------------------------------- #
class SalesAgent(Base):
    ...
    role = Column(String(50), nullable=False, default="sdr")
    data = Column(JSONB, nullable=False, server_default=text("'{}'::jsonb"), default=lambda: {})
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
class CallAnalysis(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    meeting_id = Column(Integer, ForeignKey("meetings.id"), nullable=False, index=True, unique=True)
    sales_agent_id = Column(Integer, ForeignKey("sales_agents.id"), nullable=True, index=True)
    summary = Column(Text, nullable=True)
    next_steps = Column(JSONB, nullable=False, server_default=text("'[]'::jsonb"), default=list)
    funnel_stage = Column(String(50), nullable=True)
    objections = Column(JSONB, nullable=False, server_default=text("'[]'::jsonb"), default=list)
...
class CallMetrics(Base):
    ...
    longest_monologue = Column(Float, nullable=True)
    silence_pauses = Column(Integer, nullable=True)
...
class CoachingPDI(Base):
    __tablename__ = "coaching_pdi"

    id = Column(Integer, primary_key=True, index=True)
    sales_agent_id = Column(Integer, ForeignKey("sales_agents.id"), nullable=False, index=True)
    title = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    status = Column(String(50), nullable=False, default="open")
    metrics_snapshot = Column(JSONB, nullable=False, server_default=text("'{}'::jsonb"), default=lambda: {})
    created_at = Column(DateTime, server_default=func.now())
