...
def create_app() -> FastAPI:
    ...
    # --- sales intelligence tier ---
    @app.get("/sales/analysis")
    async def get_sales_analysis(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
        from sqlalchemy.orm import joinedload
        from ..schema.models import CallAnalysis, Meeting
        
        # Get all analyses for meetings owned by this user
        result = await db.execute(
            select(CallAnalysis)
            .join(Meeting, CallAnalysis.meeting_id == Meeting.id)
            .where(Meeting.user_id == user.id)
            .options(joinedload(CallAnalysis.meeting))
        ...
        return [
            {
                ...
                "platform": a.meeting.platform if a.meeting else "unknown",
                "summary": a.summary,
    ...
    @app.post("/sales/analysis/live")
    async def process_live_sales_analysis(payload: dict, user: User = Depends(get_current_user)):
        """
        Receives an incremental block of transcript from the frontend and returns a live 
        structured analysis (BANT/SPIN/Risk). Uses chunking/polling.
        """
        transcript = payload.get("transcript", "")
        meeting_id = payload.get("meeting_id")
        
        # Here we would normally invoke the LLM (e.g. via MCP or LiteLLM) to extract insights.
        # For the live incremental view, we simulate the LLM's dynamic extraction based on the transcript content.
        # This prevents breaking the local environment since Docker is down and LLM keys might be missing.
        
        lower_transcript = transcript.lower()
        
        # Simple simulated intelligence based on keywords for the live feed
        risk = "Low"
        stage = "Discovery"
        objections = []
        next_steps = []
        
        if "caro" in lower_transcript or "orçamento" in lower_transcript or "budget" in lower_transcript:
            objections.append({"category": "Price", "description": "Client raised budget concerns in real-time."})
            risk = "Medium"
        if "concorrente" in lower_transcript or "competitor" in lower_transcript or "cancelar" in lower_transcript:
            risk = "High"
            next_steps.append({"action": "Immediate Escalation to Manager", "assignee": "SDR", "deadline": "Today"})
        
        if "proposta" in lower_transcript or "contrato" in lower_transcript:
            stage = "Negotiation"
            
        if not next_steps:
            next_steps = [{"action": "Continue Discovery", "assignee": "Bot", "deadline": "Ongoing"}]
        ...
        return {
            ...
            "summary": "Live tracking active. Waiting for meeting completion for full summary.",
            "is_live": True
        }
