...
def create_app(
    ...
    *,
    ...
    # ---- WEBHOOK INGRESS (Evolution API) ----
    # Proxied without _authorize because Evolution sends its own auth headers/payload
    @app.post("/webhooks/evolution")
    async def proxy_evolution_webhook(request: Request):
        content = await request.body()
        headers = dict(request.headers)
        # Remove host header to avoid downstream rejection
        headers.pop("host", None)
        try:
            resp = await downstream.request(
                "POST", 
                _meeting("/webhooks/evolution"), 
                headers=headers, 
                content=content
            )
    ...
    # ---- EXTERNAL API (Bots & Transcriptions) ----
    # Proxied without _authorize because it uses X-API-Key with internal secrets
    ...
    # ---- sales intelligence domain (reads from admin-api where CallAnalysis lives) ----
    @app.get("/sales/analysis")
    async def get_sales_analysis(request: Request):
        # We use _forward to inject auth (X-User-Id), ensuring users only get their own data
        return await _forward("GET", _admin("/sales/analysis"), request)

    @app.post("/sales/analysis/live")
    async def post_sales_analysis_live(request: Request):
        return await _forward("POST", _admin("/sales/analysis/live"), request)
