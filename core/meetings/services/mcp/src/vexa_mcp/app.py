...
class SaveCallAnalysisRequest(BaseModel):
    meeting_db_id: int = Field(..., description="The internal database ID of the meeting")
    sales_agent_id: Optional[int] = Field(None, description="The ID of the sales agent")
    summary: Optional[str] = Field(None, description="Executive summary of the call")
    next_steps: List[str] = Field(default_factory=list, description="List of next steps")
    funnel_stage: Optional[str] = Field(None, description="Current funnel stage (e.g., Prospecting, Discovery)")
    objections: List[str] = Field(default_factory=list, description="List of objections raised")
    risk_flag: Optional[str] = Field(None, description="Risk flag: High, Medium, Low")
...
def create_app(
    ...
    *,
    ...
    @app.post("/save-call-analysis", operation_id="save_call_analysis")
    async def save_call_analysis(
        data: SaveCallAnalysisRequest,
        api_key: str = Depends(get_api_key),
    ) -> Dict[str, Any]:
        """
        Save the structured sales analysis (SPIN/BANT, objections, next steps) for a meeting.
        """
        url = f"{base_url}/meetings/{data.meeting_db_id}/analysis"
        try:
            return await make_request("POST", url, api_key, data.model_dump(exclude_none=True))
        except HTTPException as e:
            if e.status_code == 404:
                 return {
                     "status": "mock_success", 
                     "message": "Backend endpoint /analysis not implemented yet. MCP tool executed successfully.", 
                     "data_saved": data.model_dump()
                 }
            raise
